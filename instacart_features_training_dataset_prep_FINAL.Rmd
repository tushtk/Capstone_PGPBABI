...
# add days column to orders inorder to filter out orders older than 6 months
```{r}
orders %>% 
...
# Building train/test user set; limit the size of dataset to 100 customers
```{r}
# train user set 
orders[orders$eval_set == "train",c(2,4,8)]  ->train_user
colnames(train_user)[2]  = "U_total_orders"
colnames(train_user)[3]  = "U_last_O_day"
head(train_user) 
```
# Future_features
```{r}
...
# Extract complete orders ; limit to last 10 orders
```{r}
train_user %>%
  head(100) ->train_user
train_user %>%
...
```{r}
#user_orders %>%
#  filter(U_last_O_day-O_day <=180) ->user_orders
```

#customer summary
         1. most frequent hour of the day
         2. most frequent dow
...
#  left_join(train_user[,-c(5:6)], by ="user_id") 
```

# Build prior_order files
```{r}
user_orders %>% 
  ...
  summarize(UP_prd_ord_count = n(),
          ...
          UP_ord_dow_abs_var = median(UPO_ord_dow_abs_var),
           UP_ord_hod_abs_var= median(UPO_ord_hod_abs_var),
            ...
            UP_add_to_cart_order = round(mean(add_to_cart_order/O_cart_size),2)) %>% 
...
# Building FUT order features
```{r}
# train user set 
orders[orders$eval_set == "train",c(1,2)]  ->future_ord
future_ord[future_ord$user_id %in% train_user$user_id, ]  ->future_ord
```
...
# test user set 
#orders[orders$eval_set == "test",] %>% setDT() -> test_ord
```
...
#removecols = c("order_id","FUT_order_dow","FUT_order_hod","reordered")  # #,"product_name","department","aisle")
#training_data[,!colnames(training_data) %in% removecols] ->training_data
...
# Plots 
```{r}
#moved to instacart_plots
```

# Aisle features 
```{r}
#setDT(user_prior_orders_A_D)[,lapply(.SD,first),by=c("order_id","user_id","aisle_id"),
#                .SDcols = c("order_dow","order_hour_of_day","order_dt","U_total_orders",
#                "O_reord_count","O_cart_size")] ->tmp_aisle
#setDT(user_prior_orders_A_D)[,lapply(.SD,mean),by=c("order_id","user_id","aisle_id"),
#                             .SDcols = c("add_to_cart_order")] ->tmp1_aisle
#setDT(tmp1_aisle)[,lapply(.SD,mean),by=c("user_id","aisle_id"),
#                             .SDcols = c("add_to_cart_order")] ->tmp1_aisle
#setDT(user_prior_orders_A_D)[,lapply(.SD,sum),by=c("order_id","user_id","aisle_id"),
#                             .SDcols = c("reordered")] ->tmp2_aisle
#setDT(tmp2_aisle)[,lapply(.SD,length),by=c("user_id","aisle_id"),
#                             .SDcols = c("reordered")] ->tmp2_aisle
#setDT(user_prior_orders_A_D)[,lapply(.SD,length),by=c("user_id","aisle_id"),
#                             .SDcols = c("order_id")] ->tmp3_aisle
#colnames(tmp3_aisle)[3] = "UA_num_of_orders"
```
